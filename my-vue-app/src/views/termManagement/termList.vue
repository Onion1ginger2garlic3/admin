<template>
    ...
    <table class="list">
      ...
      <tbody>
        <tr v-for="term in terms" :key="term.id">
          ...
          <td>{{ term.schoolTerm }}</td>
          <td>{{ term.schoolDate }}</td>
          <td>
            <button @click="editTerm(term)">编辑</button>
          </td>
        </tr>
      </tbody>
    </table>
  ...
  <div v-if="isAddTerm" class="modal">
    <div class="modal-content">
      <h2>添加学年学期</h2>
      <div class="item">
        <label for="schoolYear">学年：</label>
        <input id="schoolYear" v-model="FormData.schoolYear">
      </div>
      <div class="item">
        <label for="schoolTerm">学期：</label>
        <select id="schoolTerm" v-model="FormData.schoolTerm">
          <option value="第一学期">第一学期</option>
          <option value="第二学期">第二学期</option>
        </select>
      </div>
      <div class="item">
        <label for="schoolDate">时间：</label>
        <input type="date" v-model="FormData.startDate"/> 
        <input type="date" v-model="FormData.endDate"/> 
      </div>
      <div>
        <button type="button" @click="saveTerm">提交</button>
      </div>
    </div>
  </div>
</template>
...
const saveTerm = () => {
  terms.value.push({
    ...
    schoolDate: `${FormData.startDate} 至 ${FormData.endDate}`,
  });
  isAddTerm.value = false;
};
